...
try
	compilers.json = (module, filename) ->
		...
		name = filename.substr filename.lastIndexOf (if (process.platform.indexOf "win") is 0 then "\\" else "/")
		name = name.substr 1, name.indexOf(".") - 1
		module._compile "if (!window.JSONImport) window.JSONImport = {}; window.JSONImport['#{name}'] = JSON.parse(\"#{content}\", function(key, value) { var v; try { v = eval(value) } catch(e) { v = value } return v;}); ", filename
catch error
